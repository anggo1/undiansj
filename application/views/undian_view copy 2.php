...
<head>
    ...
    .draw-container {
        ...
        /* Membuat kedua panel berada di tengah halaman */
        gap: 20px;
        /* Memberikan jarak pemisah antara panel kiri dan kanan */
        max-width: 1000px;
        /* Batas maksimal lebar total gabungan kedua panel */
        margin: 40px auto;
        /* Membuat kontainer berada di tengah halaman secara horizontal */
        flex-wrap: wrap;
        /* Otomatis turun ke bawah jika dibuka di layar HP yang sempit */
    }
    ...
    * {
    ...
    .draw-panel {
        flex: 1;
        /* Membuat panel kiri fleksibel mengisi ruang */
        max-width: 460px;
        padding: 28px 24px;
    ...
    .draw-panel2 {
        flex: 1;
        /* Membuat panel kanan fleksibel mengisi ruang */
        max-width: 100%;
        padding: 28px 24px;
        border: 1px solid #312e81;
        border-radius: 18px;
...
<body>
        ...
        <div class="main-card">
            ...
            <div class="control-area">
                <select id="itemSelect">
                    ...
                    <option value="<?= (int) $item['id']; ?>" data-stock="<?= $stock; ?>"
                        <?= $stock <= 0 ? 'disabled' : ''; ?>>
            ...
            <div class="draw-container">
                ...
                <div class="draw-panel">
                    ...
                    <div
                        ...
                        <button type="button" class="draw-panel2 stop-btn" id="stopBtn" style="display:none;">⏹ STOP
                            UNDI</button>
                    </div>
            ...
            <div class="result-box" id="resultBox">
                ...
                <table style="width:100%; margin-top:5px; text-align:center;">
                    <tr>
                        ...
                        <td style="width:50%;">
                            <div class="result-nik" id="resultNik"></div>
                        </td>
                    </tr>
                    <tr>
                        <td>
            ...
            <div class="stats-grid">
                <div class="stat-card">
                    <div class="stat-label">Sisa Kandidat</div>
                    <div class="stat-value" id="candidateCount">
                        <?= isset($candidate_count) ? (int) $candidate_count : count($employees); ?></div>
                </div>
                <div class="stat-card">
                    <div class="stat-label">Total Jenis Hadiah</div>
                    <div class="stat-value"><?= count($items); ?></div>
                </div>
                <div class="stat-card">
                    <div class="stat-label">Sisa Stok Hadiah</div>
                    <div class="stat-value" id="totalStock">
                        <?php $total_stock = 0; foreach ($items as $item) { $total_stock += (int) $item['stock']; } echo $total_stock; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="history-grid" id="historyGrid">
                ...
                <div class="history-card" data-item-id="<?= (int) $item['id']; ?>">
                    <div class="history-header">
                        <div class="history-item-name"><?= htmlspecialchars($item['item_name']); ?></div>
                        <div class="history-stock" data-stock>Stok: <?= $stock; ?></div>
                    </div>
                    <div class="history-status" data-status><?= $stock > 0 ? 'Tersedia' : 'Stok Habis'; ?></div>
                    <div class="history-winners" data-winners>
                        <?php if ($nama_pemenang): ?><?= htmlspecialchars($nama_pemenang); ?><?php else: ?><span
                            class="empty-winner">Belum ada pemenang</span><?php endif; ?></div>
                </div>
                <?php endforeach; ?>
            </div>
        </div>
    ...
    <audio id="victorySound" preload="auto">
        <source src="<?= base_url('sounds/victory.mp3'); ?>" type="audio/mpeg">
    </audio>
    <audio id="drumSound" preload="auto" loop>
        <source src="<?= base_url(); ?>uploads/drum.mp3" type="audio/mpeg">
    </audio>

    <script>
    const pendingWinner =
        <?= json_encode(!empty($pending_winner) ? $pending_winner : null, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?>;
    ...
    function resetResult() {
        resultStatus.textContent = 'Pilih hadiah dan tekan Undi Nomor';
    ...
    function hideWinnerActions() {
        winnerActions.style.display = 'none';
        spinBtn.style.display = 'inline-block';
    }

    function updateCandidateCount(remainingCount) {
        candidateCount.textContent = Math.max(0, parseInt(remainingCount, 10) || 0);
    }

    function updateTotalStock() {
        let total = 0;
        itemSelect.querySelectorAll('option[data-stock]').forEach(function(option) {
            total += parseInt(option.dataset.stock || 0, 10);
        });
    ...
    function updateItemStock(itemId, stock) {
        ...
        if (option) {
            ...
            option.textContent = option.textContent.replace(/Stok:\s*\d+/, 'Stok: ' + stock);
        }
        if (card) {
            card.querySelector('[data-stock]').textContent = 'Stok: ' + stock;
            card.querySelector('[data-status]').textContent = stock > 0 ? 'Tersedia' : 'Stok Habis';
        }
        updateTotalStock();
    }
    ...
    function addWinnerToHistory(itemId, name) {
        ...
        if (empty) winners.textContent = name;
        else winners.appendChild(document.createElement('br')).parentNode.appendChild(document.createTextNode(name));
    }

    function setDrawingState(active) {
        isDrawing = active;
        spinBtn.disabled = active;
        itemSelect.disabled = active;
        spinBtn.classList.toggle('loading', active);
    }
    ...
    function finishDraw(data) {
        ...
        currentPendingWinner = {
            employee_id: data.winner_id,
            employee_number: data.winner_number || data.winner_id,
            employee_nik: data.winner_nik || '',
            employee_name: data.winner_name,
            employee_department: data.winner_dept,
